...
export default class BiMapCache<A: string|number=string,B: string|number=string> {
  ...
  constructor({key, getBfromA, getAfromB, storage, saveThrottle, maxLimit, maxAge}: Options<A,B>) {
    ...
    this._storage = storage || (global.localStorage ? global.localStorage : null);
    ...
    this._saveCache = throttle(() => {
      ...
      if (item.ids.length > maxLimit_) {
        item.ids = sortBy(item.ids, ([, , timestamp]) => timestamp)
          .slice(-maxLimit_);
      }
  ...
  _loadCache() {
    ...
    try {
      ...
      console.error('failed to load BiMapCache from storage', e); // eslint-disable-line no-console
    }
  }
